...
14. 模块Module
    ...
    - 概述
        ...
        4. 模块的加载逻辑：
            ...
            1. 预编译阶段确定模块加载顺序，按确定的顺序一次性执行整个模块代码。

                >感官上：遇到`import`时，不会跳去模块代码去执行（模块执行顺序已经在编译阶段确定），而只生成一个引用，等到需要用时，再到那个模块里去取值。
            2. 最底层被依赖的模块（最后一个被`import`的模块）最先执行。
            3. 一个模块仅能执行一次，无论被多少模块引入。
            4. “循环加载”：

                1. “循环加载”时忽略之前已经被依赖过的模块，最后加载最先出现的“循环加载”的模块。

                    >e.g. 循环依赖关系为`index->a->b->c->a`，模块执行顺序为`c->b->a->index`。
                2. “循环加载”的最先被执行模块无法获取其他模块的`import`值，仅尝试声明。
                    ...
                    >2. JS不关心是否发生了“循环加载”，只生成一个指向被加载模块的引用，需要开发者自己保证，真正取值的时候能够取到值。
            5. （除“循环加载”的第一个模块外，）模块可以提升使用`import`内容（类似变量提升，且可获得引用值）。
            6. 若被`import`的模块没有`export`，则仅执行一遍模块代码，不输出内容。
